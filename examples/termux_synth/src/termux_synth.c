...
static void trigger_note(Synth *s, float freq, float velocity, float duration) {
	...
	if (next == atomic_load_explicit(&s->tail, memory_order_acquire))
		return;
...
static void audio_callback(ma_device *device, void *output, const void *input, ma_uint32 frames) {
	...
	while (tail != head) {
		...
		for (int i = 0; i < MAX_VOICES; ++i) {
			...
			if (s->voices[i].age > s->voices[slot].age)
				slot = i;
		}
	...
	for (ma_uint32 i = 0; i < frames; ++i) {
		...
		for (int v = 0; v < MAX_VOICES; ++v)
			if (s->voices[v].active)
				sample += voice_sample(&s->voices[v], 1.f / s->sample_rate);
...
static void play_demo(Synth *s) {
	...
	for (int i = 0; i < 32 && !stopped; ++i) {
		...
		if (i % 4 == 0)
			trigger_note(s, .5f * freq, .55f, .42f);
		ma_sleep(180);
	}
...
static void play_keys(Synth *s) {
	...
	while (!stopped) {
		...
		if (read(STDIN_FILENO, &key, 1) == 1) {
			...
			if (p)
				trigger_note(s, note_freq(60 + (int)(p - keys)), .9f, .35f);
		}
		ma_sleep(8);
	}
