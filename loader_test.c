...
static void test_scheduler(void) {
	...
	out[8194] = 123;
	const tibia_api api = {.process = mock_process, .set_parameter = mock_param, .midi_msg_in = mock_midi};
	TibiaModule module = {.api = &api, .input = 1, .output = 1, .midi = 7};
	const Event events[] = {
...
static void test_stereo_scheduler(void) {
	...
	const tibia_api api = {.process = stereo_process, .set_parameter = mock_param};
	TibiaModule module = {.api = &api, .input = 2, .output = 2, .midi = -1};
...
static void test_synth(void) {
	...
	assert(e.module->midi == 0 && !e.module->input);
	assert(e.module->api->info.count == 39);
	assert(e.module->api->info.parameters[38].flags & TIBIA_PARAM_OUTPUT);
	float warmup[4096];
	int crossings;
	assert(measure(&e, &crossings) < 1e-12);
	const uint8_t note[] = {0x90, 69, 100};
	e.module->api->midi_msg_in(e.instance, e.module->midi, note);
	const uint8_t bends[][3] = {{0xe0, 0, 64}, {0xe0, 0, 0}, {0xe0, 127, 127}, {0xe0, 0, 64}};
	const int expected[] = {440, 220, 880, 440};
	for (size_t i = 0; i < 4; ++i) {
		e.module->api->midi_msg_in(e.instance, e.module->midi, bends[i]);
		render(&e, warmup, NULL, 4096);
		assert(measure(&e, &crossings) > 1e-6);
		assert(abs(crossings - expected[i]) <= 2);
	}
	const uint8_t off[] = {0x90, 69, 0};
	e.module->api->midi_msg_in(e.instance, e.module->midi, off);
	render(&e, warmup, NULL, 4096);
	assert(measure(&e, &crossings) < 1e-12);
	close_engine(&e);
...
static void test_brickworks_effect(void) {
	...
	assert(!open_engine(&e, "plugins/fx_svf/build/plugin.so"));
	assert(e.module->input == 1 && e.module->midi == -1);
