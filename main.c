...
int main(int argc, char **argv) {
	...
	const Event demo[] = {
		{0, -1, 0, {0x90, 60, 100}, 0},
		{SAMPLE_RATE, -1, 0, {0x80, 60, 0}, 0},
		{2 * SAMPLE_RATE, -1, 0, {0x90, 64, 100}, 0},
		{3 * SAMPLE_RATE, -1, 0, {0x90, 64, 0}, 0},
		{4 * SAMPLE_RATE, -1, 0, {0x90, 67, 100}, 0},
		{5 * SAMPLE_RATE, -1, 0, {0xe0, 0, 0x60}, 0},
		{6 * SAMPLE_RATE, -1, 0, {0xe0, 0, 0x40}, 0},
		{7 * SAMPLE_RATE, -1, 0, {0x80, 67, 0}, 0}
	};
	...
	if (err == MA_SUCCESS) {
		...
		for (unsigned ticks = 0; !stopped && (capture || ticks < 80); ++ticks) nanosleep(&(struct timespec){.tv_nsec = 100000000}, NULL);
		result = 0;
