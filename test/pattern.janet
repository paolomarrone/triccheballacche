...
(def repeating (p/parallel [(p/loop crossing) (p/loop (p/steps 3 [70 nil 72]))]))
(defn event-set [items]
  (def found @{})
  (each item items (put found (item :id) item))
  (freeze found))
(def whole (event-set (p/query repeating 0 17)))
(def parts @[])
(each [a b] [[0 0.3] [0.3 2] [2 2.1] [2.1 8] [8 17]]
  (each item (p/query repeating a b) (array/push parts item)))
(assert (deep= whole (event-set parts)))
(assert (deep= (p/query repeating 1000000 1000004) (p/query repeating 1000000 1000004)))
(assert (= (length (p/query repeating 2 2)) 0))
(def tail (p/query (p/loop (p/events 2 [[1 3 :tail]])) 2 2.5))
(assert (= ((tail 0) :start) 1))
(assert (= ((tail 0) :end) 3))
(def delayed (p/serial [(p/steps 2 [nil]) (p/loop motif)]))
(assert (= (length (p/query delayed 0 2)) 0))
(assert (= (((p/query delayed 2 2.25) 0) :start) 2))
(assert (deep= (p/query (p/map identity repeating) 0 8) (p/query repeating 0 8)))
(rejects (fn [] (p/serial [(p/loop motif) motif])))
(rejects (fn [] (p/loop empty)))
(rejects (fn [] (p/loop repeating)))
(rejects (fn [] (p/reverse repeating)))
(rejects (fn [] (p/query repeating 0 100 1)))
(rejects (fn [] (p/query (p/loop motif) 1e100 2e100)))
(print "OK: periodic sources, independent periods, window partition invariance, whole notes, stable IDs and query bounds")
