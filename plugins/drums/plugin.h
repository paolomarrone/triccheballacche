...
static size_t plugin_mem_req(void *p) {
	(void)p;
	return 0;
}

static void plugin_mem_set(void *p, void *mem) {
	(void)p;
	(void)mem;
}
...
static void plugin_reset(void *p) {
	Drums *d = p;
	memset(d->voices, 0, sizeof(d->voices));
	d->rng = (uint32_t)d->seed;
}
...
static void plugin_midi_msg_in(void *p, size_t bus, const uint8_t *msg) {
	(void)bus;
	Drums *d = p;
	if ((msg[0] & 0xf0) != 0x90 || !msg[2] || msg[1] > TOM_LOW || msg[2] > 127)
		return;
	const float duration[] = {.42f, .32f, .085f, .38f, 1.6f, .3f, .4f};
	Voice *v = d->voices;
	for (int i = 0; i < VOICES; ++i) {
		if (d->voices[i].age >= d->voices[i].length) {
			v = d->voices + i;
			break;
		}
		if (d->voices[i].age > v->age)
			v = d->voices + i;
	}
	*v = (Voice){.length = (size_t)(duration[msg[1]] * d->rate),
	    .type = msg[1],
	    .strength = msg[2] / 127.f,
	    .seed = random_u32(&d->rng)};
}
...
static void plugin_process(void *p, const float **in, float **out, size_t n) {
	...
	for (size_t i = 0; i < n; ++i) {
		...
		for (int k = 0; k < VOICES; ++k) {
			...
			v->low += .17f * (white - v->low);
			float high = white - v->low;
			if (type == KICK) {
				...
				float metal = sinf(tau * 4177.f * t + 2.f * sinf(tau * 683.f * t)) + .5f * sinf(tau * 7319.f * t) +
				    .3f * sinf(tau * 10331.f * t);
