...
Module *load_module(const char *path, const PluginConfig *config) {
	...
	if (config->input < 0 || config->input > 2 || config->output < 1 || config->output > 2 ||
	    config->inputs < config->input || config->inputs > MAX_INPUTS || config->input_offset < 0 ||
	    config->input_offset > MAX_INPUTS || config->input_offset + config->input > config->inputs ||
	    config->midi < -1 || config->nparams < 0 || config->nparams > MAX_PARAMS)
		goto fail;
	m->config = *config;
	...
	*slash = 0;
	...
	for (int i = 0; i < config->nparams; ++i)
		if ((config->outputs & (UINT64_C(1) << i)) ? !a->get_parameter
		                                           : (!a->set_parameter || !isfinite(config->defaults[i])))
			goto fail;
	return m;
fail:
